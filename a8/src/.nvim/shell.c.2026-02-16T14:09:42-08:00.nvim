...
static void build_prompt(void) {
  char cwd[PROMPT_SIZE - 4];
  if (!getcwd(cwd, sizeof(cwd))) {
    fprintf(stderr, "shell: " GETCWD_ERROR_MSG "\n");
...
static void help_print_all_stdout(void) {
  printf(FORMAT_MSG("help", HELP_HELP_MSG));
  printf(FORMAT_MSG("exit", EXIT_HELP_MSG));
  printf(FORMAT_MSG("pwd", PWD_HELP_MSG));
  printf(FORMAT_MSG("cd", CD_HELP_MSG));
  printf(FORMAT_MSG("history", HISTORY_HELP_MSG));
  fflush(stdout);
}

static void help_print_all_signal_safe(void) {
  write_all(STDOUT_FILENO, FORMAT_MSG("help", HELP_HELP_MSG));
  write_all(STDOUT_FILENO, FORMAT_MSG("exit", EXIT_HELP_MSG));
  write_all(STDOUT_FILENO, FORMAT_MSG("pwd", PWD_HELP_MSG));
  write_all(STDOUT_FILENO, FORMAT_MSG("cd", CD_HELP_MSG));
  write_all(STDOUT_FILENO, FORMAT_MSG("history", HISTORY_HELP_MSG));
}

static void sigint_handler(int signo) {
  (void)signo;
  write_all(STDOUT_FILENO, "\n");
  help_print_all_signal_safe();
...
static int do_pwd(ParsedCommand *pc) {
  if (pc->argc > 1) {
    fprintf(stderr, "pwd: " TMA_MSG "\n");
    return 0;
  }
  char cwd[PROMPT_SIZE];
  if (!getcwd(cwd, sizeof(cwd))) {
    fprintf(stderr, "pwd: " GETCWD_ERROR_MSG "\n");
    return 0;
  }
  printf("%s\n", cwd);
  fflush(stdout);
...
static int do_cd(ParsedCommand *pc, char *prev_dir, size_t prev_sz) {
  ...
  if (pc->argc == 1) {
    target = get_home_dir();
    if (!target) {
      fprintf(stderr, "cd: " CHDIR_ERROR_MSG "\n");
      return 0;
    }
  } else {
...
static int do_help(ParsedCommand *pc) {
  if (pc->argc > 2) {
    fprintf(stderr, "help: " TMA_MSG "\n");
    return 0;
  }
  if (pc->argc == 1) {
    help_print_all_stdout();
    return 0;
  }
...
static int run_external(ParsedCommand *pc) {
  pid_t pid = fork();
  if (pid < 0) {
    fprintf(stderr, "shell: " FORK_ERROR_MSG "\n");
    return 0;
  }

  if (pid == 0) {
    execvp(pc->argv[0], pc->argv);
    fprintf(stderr, "shell: " EXEC_ERROR_MSG "\n");
    _exit(127);
  }
  ...
  while (1) {
    ...
    if (w < 0) {
      if (errno == EINTR)
        continue;
      fprintf(stderr, "shell: " WAIT_ERROR_MSG "\n");
      break;
    }
  }
  return 0;
...
static int execute_line(const char *cmdline, History *hist, char *prev_dir,
  ...
  if (strcmp(pc.argv[0], "history") == 0) {
    if (pc.argc > 1) {
      fprintf(stderr, "history: " TMA_MSG "\n");
    } else {
      history_print(hist);
      fflush(stdout);
    }
...
int shell_run(void) {
  ...
  while (1) {
    ...
    if (n < 0) {
      if (errno == EINTR)
        continue;
      fprintf(stderr, "shell: " READ_ERROR_MSG "\n");
      continue;
    }
    if (n == 0)
      break;
    ...
    if (line[0] == '!') {
      if (strcmp(line, "!!") == 0) {
        ...
        if (!history_get_last(&hist, &cmd)) {
          fprintf(stderr, "history: " HISTORY_NO_LAST_MSG "\n");
          continue;
        }
        printf("%s\n", cmd);
        fflush(stdout);
      ...
      if (!is_all_digits(numstr)) {
        fprintf(stderr, "history: " HISTORY_INVALID_MSG "\n");
        continue;
      }
